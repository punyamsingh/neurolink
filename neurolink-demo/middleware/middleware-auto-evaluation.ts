...
const callback = (evaluation: EvaluationData) => {
  console.log("Auto-evaluation middleware invoked with evaluation:", evaluation);
};
...
const negativeResult = await provider.generate({
  prompt: "If z = 2 - 3i + 4i^2 + 641451134, then find z^2. Do it one step only ", // Intentionally incorrect statement
